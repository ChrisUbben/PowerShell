...
Describe "Extended FileSystem Item/Content Cmdlet Provider Tests" -Tags "Feature" {
    ...
    Context "Valdiate New-Item parameters for the FileSystem provider" {
        BeforeEach {
    ...
    Context "Valdiate Get-Item parameters for the FileSystem provider" {
        It "Verify Force" {
    ...
    Context "Valdiate Move-Item parameters for the FileSystem provider" {
        BeforeAll {
    ...
    Context "Valdiate Rename-Item parameters for the FileSystem provider" {
        BeforeAll {
            $newFile = "NewName.txt"
        }
    ...
    Context "Valdiate Remove-Item parameters for the FileSystem provider" {
        It "Verify WhatIf" {
    ...
    Context "Valdiate Set-Content parameters for the FileSystem provider" {
    ...
    Context "Valdiate Get-Content parameters for the FileSystem provider" {
        BeforeEach {
...
Describe "Extended FileSystem Path/Location Cmdlet Provider Tests" -Tags "Feature" {
    ...
    Context "Validate Get-ChildItem Cmdlet Parameters" {
        It "Validate Path" {
            $result = @(Get-ChildItem -Path $level1_0)
            $result[0].Name | Should Be $level2_0
            $result[1].Name | Should Be $level2_1
        }

        It "Validate Filter" {
            $result = Get-ChildItem -Path "$level1_0Full\*" -Filter "*2_0"
            Write-Verbose $result.Name -Verbose
            $result.Name | Should Be $level2_0
        }

        It "Validate Include" {
            $result = Get-ChildItem -Path "$level1_0Full\*" -Include "*2_0"
            dir $level1_0Full\* | Write-Verbose -Verbose
            $result.Name | Should Be (Resolve-Path $level2_0Full)
        }

        It "Validate Include and Exclude Intersection" {
            $result = Get-ChildItem -Path "$level1_0Full\*" -Include "Level*" -Exclude "*2_0"
            Write-Verbose $result.Name -Verbose
            $result.Name | Should Be $level2_1
        }
    }
